...
class AstyxDataset(DatasetTemplate):
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            info = {}
            pc_info = {'num_features': 4, 'radar_idx': sample_idx}
            ...
            if has_label:
                ...
                gt_boxes_radar = np.array([[*obj.loc, obj.w, obj.l, obj.h, obj.rot] for obj in obj_list])
                annotations['gt_boxes_radar'] = gt_boxes_radar
                ...
                if count_inside_pts:
                    ...
                    corners_radar = box_utils.boxes_to_corners_3d(gt_boxes_radar)
                    num_points_in_gt = -np.ones(num_gt, dtype=np.int32)

                    for k in range(num_objects):
                        flag = box_utils.in_hull(points[:, 0:3], corners_radar[k])
            ...
            for i in range(num_obj):
                ...
                if (used_classes is None) or names[i] in used_classes:
                    ...
                    db_info = {'name': names[i], 'path': db_path, 'image_idx': sample_idx, 'gt_idx': i,
                               'box3d_radar': gt_boxes[i], 'num_points_in_gt': gt_points.shape[0],
    ...
    def generate_prediction_dicts(batch_dict, pred_dicts, class_names, output_path=None):
        ...
        def get_template_prediction(num_samples):
            ret_dict = {
                ...
                'score': np.zeros(num_samples), 'boxes_radar': np.zeros([num_samples, 7])
            }
            return ret_dict
        ...
        def generate_single_sample_dict(batch_index, box_dict):
            ...
            for i, obj in enumerate(obj_list):
                ...
                pred_dict['alpha'][i] = -np.arctan2(obj.loc[1], obj.loc[0]) + obj.rot_camera
                obj.from_radar_to_image(calib)
                pred_dict['bbox'][i, :] = np.array(obj.box2d)

            pred_dict['name'] = np.array(class_names)[pred_labels - 1]
            pred_dict['score'] = pred_scores
            pred_dict['boxes_radar'] = pred_boxes
